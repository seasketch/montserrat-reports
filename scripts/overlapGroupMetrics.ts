...
import { overlapPoints } from "./overlapPoints";
...
function isFeaturePointArray(
  ...
  // If the array is empty, we can't determine the type, so return false
  return false;
}
 ...
 */
export async function overlapPointsGroupMetrics(options: {
  ...
  return overlapGroupMetrics({
    ...
    operation: async (
      ...
      const overallGroupMetrics = await overlapPoints(metricId, features, sc, {
